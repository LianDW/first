 ...
 **/
public class TestReadExcel extends DefaultHandler {
    ...
    public static void main(String[] args) {
		...
		try {
			t.process("E:/我的ur/new中信保单登记/测试文件/测试数据_再保数据文件格式-v2.2_造数据版本.xlsx");
    ...
    public void endElement(String uri, String localName, String name) throws SAXException {
    	...
    	if(sheetIndex==5){//第五页不要
    		return;
    	}
    	if(ref!=null){
    		if("A".equals(ref.replaceAll("\\d+", ""))){//第一列不要
    			return;
    		}
    		if("B".equals(ref.replaceAll("\\d+", ""))){
    			if((lastIndex.trim()).isEmpty()){//第二列行号如果是空，跳过
    				return;
    			}
    			if ("v".equals(name)&&"".equals(this.getDataValue(lastIndex.trim(), "").trim())) {
    				return;
    			}
    		}
    	}
        ...
        if (isTElement) {//这个程序没经过
            ...
            cellList.add(curCol, value.trim());
            curCol++;
            //如果里面某个单元格含有值，则标识该行不为空行
            if (value != null && !"".equals(value)) {
                flag = true;
            ...
            if ("row".equals(name)) {
                ...
                if (flag&&curRow>=5){ //该行不为空行且该行不是第一行，则发送（第一行为列名，不需要）
